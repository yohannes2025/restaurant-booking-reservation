...
# --- New PDF and Email Helper Function ---
...
def send_booking_confirmation_email(booking):
    """
    Sends a confirmation email to the user with booking details and a PDF attachment.
    """
    try:
        subject = f"Your Booking Confirmation for {booking.booking_date} at {booking.booking_time}"
        to_email = booking.user.email
        from_email = "your_restaurant_email@example.com" # TODO: Replace with your actual sender email

        # Render HTML content for the email body
        email_html_message = render(None, 'bookings/booking_confirmation_email.html', {'booking': booking}).content.decode('utf-8')

        # Generate PDF
        pdf = render_to_pdf('bookings/booking_confirmation_pdf.html', {'booking': booking})
        ...
        if pdf:
            ...
            email.attach(f"booking_confirmation_{booking.id}.pdf", pdf, 'application/pdf')
            email.send()
            print(f"DEBUG: Confirmation email sent to {to_email}")
        else:
            print(f"ERROR: Could not generate PDF for booking {booking.id}")
            # Optionally send an email without PDF or log for administrative review
            email = EmailMessage(
                subject,
                email_html_message,
                from_email,
            ...
            email.send()
            print(f"DEBUG: Confirmation email sent without PDF to {to_email}")

    except Exception as e:
        print(f"ERROR: Failed to send confirmation email for booking {booking.id}: {e}")
            ...
            if available_tables.exists():
                ...
                try:
                    with transaction.atomic():
                        ...
                        # --- End Send Confirmation Email with PDF ---
            ...
            if available_tables.exists():
                ...
                try:
                    with transaction.atomic():
                        ...
                        messages.success(
                            request, f"Your booking for Table {selected_table.number} has been updated successfully!")
                        # You might want to send an updated confirmation email here too
                        # send_booking_confirmation_email(booking) 
                        return redirect('my_bookings')
                except Exception as e:
                    messages.error(
